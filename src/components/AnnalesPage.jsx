const ANNALES = [
  ...
  { year:"2019", title:"Sujet BEPC 2019", chapitres:["Géométrie","Algèbre","Probabilités"] },
];
function getScore(annale, chapitres) {
  if (!chapitres?.length) return null;
  const scores = annale.chapitres.map(ch => chapitres.find(c=>c.name===ch)?.score??null).filter(s=>s!==null);
  return scores.length ? Math.round(scores.reduce((a,b)=>a+b,0)/scores.length) : null;
}
function scoreClass(s) { if(s===null)return"none"; if(s>=70)return"good"; if(s>=50)return"mid"; return"low"; }
    ...
    <div className="annales-page" style={{animation:"fade-in .4s ease"}}>
      <div className="annales-card">
        ...
        {ANNALES.map(a => {
          ...
          return (
            <div key={a.year} className="annale-item" onClick={() => { onStartExam(a.chapitres[0]); showToast(`Sujet BEPC ${a.year} — Bonne chance !`); }}>
              <div className="annale-year">{a.year}</div>
              <div className="annale-info"><div className="annale-title-text">{a.title}</div><div className="annale-tags">{a.chapitres.join(" · ")}</div></div>
              <div className={`annale-score ${scoreClass(s)}`}>{s!==null?`${s}%`:"—"}</div>
              <button className="annale-start-btn">Commencer →</button>
            </div>
          );
        })}
